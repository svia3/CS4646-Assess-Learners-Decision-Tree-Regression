...
class DTLearner():
	...
	def absCorrelation(self, data):
		...
		for i in range(0,xData.shape[1]):
			if len(set(xData[:,i])) != 1:
				x = np.corrcoef(x=xData[:,i], y=yData, rowvar=True) #inputs->(x values, y values)
				splitCoeffs.append(x[0,1]) # x correlation with y (top right corner of 2x2)
		# print(splitCoeffs)
		return np.argmax(splitCoeffs) #index of the max -> all positive 
	...
	def buildTree(self, data):
		...
		i = self.absCorrelation(data)	 # best feature 
